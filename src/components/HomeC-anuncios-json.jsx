...
let API_anuncios = "https://api.lojasdomago.com.br";
if(window.location.href.includes("localhost")) {
  API_anuncios = "http://api.lojasdomago.com.br";
}
...
export default class HomeC extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      filtrandoAnuncioPorCategoria: "todos",
      filtrandoAnuncioPorOrdem: "name",
      soma_filtrado: 1,
    };
  }
  ...
  handleOrder = (event) => {
    this.setState({ filtrandoAnuncioPorOrdem: event.target.value });
  };
  ...
  render() {
    ...
    if (this.state.filtrandoAnuncioPorOrdem.toLowerCase() === "name") {
      anuncios.sort((a, b) => a.name > b.name ? 1 : -1)
    } else if (this.state.filtrandoAnuncioPorOrdem.toLowerCase() === "price") {
      anuncios.sort((a, b) => a.price > b.price ? 1 : -1)
    }
    /*.filter( (data) => {
      if (this.state.filtrandoAnuncioPorOrdem.toLowerCase() === "name") {
        return anuncios.sort((a, b) => a.name > b.name ? 1 : -1);
      } else if (this.state.filtrandoAnuncioPorOrdem.toLowerCase() === "price") {
        return anuncios.sort((a, b) => a.price > b.price ? 1 : -1);
      }      
    })*/
    this.state.soma_filtrado = anuncios.reduce(
      (accumulator, {price} ) => 
      accumulator + parseFloat(price), 0
    );
    ...
    return (
      <div>
        <div className="header-filtro">
          <div className="row my-2">
            <div className="col-md-10 offset-1 mt-1 mb-2 flex all-center">
              ...
              />
              <svg className="float-end" style={{top:"-30px", position: "relative", right:"10px"}} viewBox="0 0 24 24" width="24" height="24" size="24" color="currentColor"><path fill="currentColor" fillRule="evenodd" d="M16.84 15.78l4.69 4.69a.75.75 0 01-1.06 1.06l-4.69-4.69a8.25 8.25 0 111.06-1.06zm-1.51-.564a6.75 6.75 0 10-.113.113.759.759 0 01.112-.113z"></path></svg>
            </div>
          </div>
        ...
        <div className="row">
          <div className="col">
            Som: R${ this.state.soma_filtrado }
            </div>
        </div>
      </div>
   );
